...
echo "Processes,Size,Latency" > gather2_fixed_node.csv
...
do
    ...
    result_bcast=$(mpirun --map-by node -np $processes --mca coll_tuned_use_dynamic_rules true --mca coll_tuned_bcast_algorithm 2 osu_gather -m $size -x $repetitions -i $repetitions | tail -n 1 | awk '{print $2}')
	
    echo "$processes, $size, $result_bcast"
    # Scrivi i risultati nel file CSV
    echo "$processes,$size,$result_bcast" >> gather2_fixed_node.csv
