...
echo "Processes,Latency" > output_processes_fixed_size_bcast_2_new.csv
...
do
  ...
  result_bcast=$(mpirun --map-by core -np $processes --mca coll_tuned_use_dynamic_rules true --mca coll_tuned_bcast_algorithm 2 osu_bcast -m 2 -x $repetitions -i $repetitions | tail -n 1 | awk '{print $2}')
